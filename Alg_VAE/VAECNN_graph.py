...
class VAECNNGraph(BaseGraph):
    def __init__(self, network_params,sigma=0.001, sigma_act=tf.nn.softplus,
        ...
        self.state_dim = network_params.state_dim
        ...
        self.cell_type=network_params['cell_type']
    ...
    def create_TVAE(self):
        ...
        eps = tf.random_normal(tf.shape(self.decoder_mean), 0, 1, dtype=tf.float32)
        self.decoder_x = tf.add(self.decoder_mean, tf.multiply(tf.sqrt(self.sigma), eps))
        ...
        with tf.variable_scope('decoder_mean', reuse=True):
            Px_z_mean = DenseNet(input_=self.z_sampled,
                            hidden_dim=self.hidden_dim, 
                            output_dim=self.x_flat_dim, 
                            num_layers=2, 
                            transfer_fct=self.transfer_fct,
        ...
        eps = tf.random_normal(tf.shape(self.samples_mean), 0, 1, dtype=tf.float32)
        self.samples = tf.add(self.samples_mean, tf.multiply(tf.sqrt(self.sigma), eps))
    ...
    def create_loss_optimizer(self):
        ...
        with tf.name_scope('reconstruct'):
            ...
            self.reconstruction = -0.5 / self.sigma * tf.reduce_sum(tf.square(self.decoder_mean - self.x_batch_flat), 1)
        ...
        with tf.variable_scope("optimizer" ,reuse=self.reuse):
            ...
            self.optimizer = tf.train.AdamOptimizer(self.learning_rate, beta1=0.99)
