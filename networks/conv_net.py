...
class ConvNet(object):
    ...
    def conv_layer(self, input_, output_dim, name, act_func=tf.nn.relu):
        conv = tf.layers.conv2d(inputs=input_, filters=filters, kernel_size=k_size, strides=strides, 
                                padding=padding, activation=act_func, kernel_initializer=self.kinit, 
                                bias_initializer=self.bias_unit, name=name, reuse=self.reuse)
