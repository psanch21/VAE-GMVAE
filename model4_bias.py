...
'''
This file contains the GMVAE model with truncated initialization for bias in the z output layer
'''
...
# TODO: xavier_init
# def_init = tf.truncated_normal_initializer(stddev=0.05)
# kernel_init = tf.contrib.layers.variance_scaling_initializer()
# def_init = tf.contrib.layers.xavier_initializer()
# TODO: Not in use
...
def Qz_x(x, z_dim, args, reuse, rate):
    ...
    with tf.variable_scope('Qz_x_mean'):
        z_mean, h_z_mean = utils.dense_network_biased(x, z_dim, args['hidden_dim'], args['num_layers'],reuse, rate, out_act= act_func_mean, std_bias=std_bias)
        z_mean = tf.scalar_mul(max_value,z_mean)

    # VARIANCE
    with tf.variable_scope('Qz_x_var'):
        z_var_aux, h_z_logvar = utils.dense_network(x, z_dim, args['hidden_dim'], args['num_layers'], reuse, rate, out_act=act_func_var)
...
def Qw_x(x, w_dim, args, reuse, rate):
    ...
    with tf.variable_scope('Qw_x_mean'):
        w_mean, h_w_mean = utils.dense_network(x, w_dim, args['hidden_dim'], args['num_layers'],reuse, rate)
    # VARIANCE
    with tf.variable_scope('Qw_x_var'):
          w_var_aux, h_w_var = utils.dense_network(x, w_dim, args['hidden_dim'], args['num_layers'], reuse, rate, out_act=tf.tanh)
          w_var = tf.nn.softplus(w_var_aux)
    return w_mean, w_var
...
def Pz_wy(w, z_dim, reuse, rate,K_clusters, hidden_dim=64, num_layers=2):
    ...
    with tf.variable_scope('Pz_wy'):
        h_out, _ = utils.dense_network(w, hidden_dim, hidden_dim, num_layers,reuse, rate, out_act= tf.nn.relu)
...
def Px_z(z, data_dim, hidden_dim, reuse, rate):
    ...
    with tf.variable_scope('Px_z'):
        x_mean, _ = utils.dense_network(z, data_dim, hidden_dim, 2, reuse, rate, out_act=tf.nn.sigmoid)
...
def new_samples(num_imgs,z_dim, w_dim, output_dim, args, rate):
    ...
    z_means, z_vars  = Pz_wy(eps_sample, z_dim, True, rate,args['K_clusters'], hidden_dim=args['hidden_dim'], num_layers=args['num_layers'])
    samples = list()
...
def graph(shape, reuse, rate, args):
    ...
    print('\n[*] Defining Px_z...')
    x_decoded_mean = Px_z(z, data_dim, hidden_dim, reuse, rate)


    print('\n[*] Defining Sampling...')
    # REMEMBER WE ARE REUSING THE DECODER NETWORK
    samples = new_samples(num_imgs,z_dim, w_dim, data_dim, args, rate)
    ...
    with tf.name_scope('reconstruct'):
        reconstruction = -0.5 / sigma_recons * tf.reduce_sum(tf.square(x_decoded_mean - x), 1)
